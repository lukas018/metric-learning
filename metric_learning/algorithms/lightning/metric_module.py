...
@dataclass
class PreTrainArguments(DataclassFromDict):
    lr: float = 0.1
...
@dataclass
class FewshotArguments(DataclassFromDict):
    lr: float = 0.001
    ways: int = 5
    shots: int = 1
    queries: int = 15
...
class LightningFewshotModule(pl.LightningModule):
    ...
    def _support_query_split(
        ...
        return support, query, query_labels.to(support.device)

    def _meta_step(self, batch) -> tuple[torch.Tensor, torch.Tensor]:
        support, query, query_labels = self._support_query_split(batch)
        logits = self.learner(query, support)
