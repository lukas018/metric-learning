...
# Copied from https://raw.githubusercontent.com/WangYueFt/rfs/master/models/resnet.py
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.layer1 = self._make_layer(
            block, n_blocks[0], 64, stride=2, drop_rate=drop_rate
        )
        self.layer2 = self._make_layer(
            block, n_blocks[1], 160, stride=2, drop_rate=drop_rate
        )
    ...
    def _make_layer(
        ...
        if n_block == 1:
            ...
            layer = block(
                self.inplanes, planes, stride, downsample, drop_rate, self.use_se
            )
        layers.append(layer)
        ...
        for i in range(1, n_block):
            if i == n_block - 1:
                ...
                layer = block(
                    self.inplanes, planes, drop_rate=drop_rate, use_se=self.use_se
                )
            layers.append(layer)
...
def resnet12(keep_prob=1.0, avg_pool=False, **kwargs):
    ...
    model = ResNet(
        BasicBlock, [1, 1, 1, 1], keep_prob=keep_prob, avg_pool=avg_pool, **kwargs
    )
    return model
...
def resnet18(keep_prob=1.0, avg_pool=False, **kwargs):
    ...
    model = ResNet(
        BasicBlock, [1, 1, 2, 2], keep_prob=keep_prob, avg_pool=avg_pool, **kwargs
    )
    return model
...
def resnet24(keep_prob=1.0, avg_pool=False, **kwargs):
    ...
    model = ResNet(
        BasicBlock, [2, 2, 2, 2], keep_prob=keep_prob, avg_pool=avg_pool, **kwargs
    )
    return model
...
def resnet50(keep_prob=1.0, avg_pool=False, **kwargs):
    ...
    """
    model = ResNet(
        BasicBlock, [3, 4, 6, 3], keep_prob=keep_prob, avg_pool=avg_pool, **kwargs
    )
    return model
...
def resnet101(keep_prob=1.0, avg_pool=False, **kwargs):
    ...
    """
    model = ResNet(
        BasicBlock, [3, 4, 23, 3], keep_prob=keep_prob, avg_pool=avg_pool, **kwargs
    )
    return model
...
if __name__ == "__main__":
    ...
    model = model_dict[args.model](
        avg_pool=True, drop_rate=0.1, dropblock_size=5, num_classes=64
    )
