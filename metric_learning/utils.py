...
class HfArgumentParser(ArgumentParser):
    ...
    def __init__(
        ...
        dataclass_types: Union[
            DataClassType, Iterable[DataClassType], dict[DataClassType, str]
        ],
        **kwargs,
    ):
        """
        ...
        for dtype in self.dataclass_types:
            prefix = (
                None
                if not isinstance(self.dataclass_types, dict)
                else self.dataclass_types.get(dtype)
            )
            self._add_dataclass_arguments(dtype, prefix=prefix)

    def _add_dataclass_arguments(
        self, dtype: DataClassType, prefix: Optional[str] = None
    ):
        for field in dataclasses.fields(dtype):
            if not field.init:
                continue

            _field_name = field.name if prefix is None else f"{prefix}_{field.name}"
    ...
    def parse_args_into_dataclasses(
        ...
        use_prefix = isinstance(self.dataclass_types, dict)

        for dtype in self.dataclass_types:
            prefix = self.dataclass_types.get(dtype) if use_prefix else None
...
def parse_arg_file(path_to_args: str):
    ...
    pretrain_args = args_dict.get("pre_training", {})
    pretrain_args = PreTrainArguments.from_dict(pretrain_args)
    fs_args = FewshotArguments.from_dict(args_dict.get("fewshot_training", {}))
